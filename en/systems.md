...
- **Debugging a distributed bug with AI** — hand the model the trace, the real code path, and what you've ruled out, then ask for ranked hypotheses and the cheapest confirmation — not "fix it." → [Debugging with AI](/debugging-with-ai-hand-it-the-evidence/)

- **Designing a service with AI as interlocutor** — bring your own design and have the model attack it (where the boundary leaks, what breaks under partial failure), argue both sides of a trade-off, and keep the pen. → [Designing a service](/designing-a-service-with-ai-as-interlocutor/)

- **Model choice as an engineering decision** — match the tier and the effort dial to the task instead of defaulting to the biggest model; decide on a real eval set, not on instinct. → [Picking a model](/picking-the-right-model-for-the-task/)
...
- **Multi-agent fan-out, costed honestly** — when N parallel agents beat one (independent, wide work) and when they just multiply the bill; decompose by genuine independence and budget the merge. → [When the fan-out pays for itself](/multi-agent-orchestration-when-its-worth-it/)
...
- **Reviewing AI code for its failure modes** — AI code is *plausibly* wrong, not obviously wrong; a checklist for the seams it can't see — error paths, blast radius beyond the diff, concurrency, scope creep. → [Reviewing AI-generated code](/reviewing-ai-generated-code-before-you-trust-it/)

- **Green tests that prove nothing** — AI-written tests pass by asserting the mock or snapshotting the bug as expected; write the failing test first and read every assertion for can-it-fail. → [Testing with AI](/testing-with-ai-green-doesnt-mean-tested/)

- **The day AI nearly shipped a bug to prod** — a clean diff, a green suite, and a confident explanation are one source agreeing with itself; the independent check has to come from outside the model's own loop. → [What I changed](/the-day-ai-nearly-shipped-a-bug-to-prod/)
